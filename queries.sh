...
echo "$($PSQL "SELECT DISTINCT team_name
              FROM (
                  SELECT teams.name AS team_name FROM games
                  LEFT JOIN teams ON games.winner_id = teams.team_id
                  WHERE games.round = 'Eighth-Final' AND games.year = 2014
                  UNION
                  SELECT opponent_teams.name AS team_name FROM games
                  LEFT JOIN teams AS opponent_teams ON games.opponent_id = opponent_teams.team_id
                  WHERE games.round = 'Eighth-Final' AND games.year = 2014
              ) AS team_names
              ORDER BY team_name")"
